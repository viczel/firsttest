...
foreach ($aNewData As $k =>$arr) {
    foreach($arr As $id) {
        ...
        addUpdateRequest($oBatchSender, $configList[$k], $ob);
    }
}
 ...
 * @param string $finish
 */
function addUpdateRequest(BatchSender $oSender, Config $config, $ob) {
    $oSender->addRequest($config, 'put', '//bs-core/main/leads/' . $ob->id, $ob);
}
